...
| Atomic Multi-Store Sync Pattern | When multiple Zustand stores must stay synchronized: (1) Use `getState()` for synchronous access, (2) Create direct function object (`atomicBatchActions`) as single source of truth, (3) Hook delegates to direct function for DRY, (4) DEV logging goes in direct function (shared entry point). Prevents race conditions where effects observe intermediate state between store updates (14e-34b) |
| Settings Store Extension Pattern | Extend existing Zustand stores with new state: (1) Add state fields to existing interface, (2) Add actions following existing naming convention, (3) Add individual selectors for render isolation (useLang, useCurrency), (4) Add combined useShallow selector for related fields (useLocaleSettings), (5) Maintain backward compatibility via dual writes to plain localStorage keys, (6) Update persist merge function for legacy key migration. Pattern enables incremental store consolidation without breaking consumers (14e-35) |
| Nested Directory Staging Pattern | When story creates NEW subdirectories in existing features (e.g., `src/features/categories/utils/` AND `tests/unit/features/categories/utils/`), each directory tree shows as single `??` in git status. Must `git add` EACH directory separately. Extends 14e-36a pattern (top-level feature directories) to nested utils/tests directories (14e-42) |
| Pure Utility Extraction with DI | Extract business logic as pure function with dependency injection: (1) Create pure function in `utils/` subdirectory, (2) Accept hook results as parameters (e.g., `FindItemNameMatchFn`), (3) Return immutable result object, (4) Caller creates wrapper that binds dependencies. Pattern enables comprehensive unit testing without mocking hooks/context. Remove any duplicates - single source of truth (14e-42: `applyItemNameMappings`) |
...
| Incomplete Prop Removal | When removing a prop from a hook call, ALSO remove the destructure declaration - TypeScript TS6133 "declared but never read" breaks build. Use `_` prefix if keeping for future use (14e-34b code review fix) |
...
| Zustand store split pattern | Store creation stories should split: foundation+actions (a) → selectors+tests (b) → migration (c). Pattern from 14e-6a/b/c/d, 14e-12a/b, now 14e-36a/b/c. 5pt store story with 6 tasks + 29 subtasks → 3x 2pt stories = 6pt total but manageable context windows |
...
|------|-------------------|
| 2026-02-01 | ProcessScan Store Direct Access Pattern (14e-43 APPROVED: Replace UI callback injection with Zustand store direct access. Handler imports action objects `scanActions`, `transactionEditorActions` not hooks. Non-React code uses `scanActions.processError()` instead of `ui.setScanError()`. Only callbacks with no store equivalent remain injected (`setToastMessage`). Test pattern: Use `vi.hoisted()` to create mock action objects BEFORE `vi.mock()` factories run - mock entire store module. Documentation pattern: Mark deprecated callbacks with `@deprecated Story 14e-43:` JSDoc including replacement and "This callback is ignored" note), Multi-file deletion story staging disaster pattern (14e-45 atlas-code-review: 12 files across story/deleted/modified/mixed categories ALL unstaged - story file `??`, deleted files ` D`, modified files ` M`, mixed `MM`. Extends 14e-33/14e-28/14e-25b1 patterns. When story involves file DELETIONS, `git rm` or `git add` required to stage deletions - deleted files show ` D` (unstaged) vs `D ` (staged). Pattern: After ALL story work complete, run `git status --porcelain` and verify EVERY claimed file has correct staged prefix: `A ` (added), `M ` (modified), `D ` (deleted), `R ` (renamed). NO space before letter = staged. Fix: `git add <all-story-files>` stages adds/mods/deletes atomically), NavigationContext→Zustand migration pattern (14e-45 APPROVED: Delete React Context when Zustand store is superset. Pattern: (1) Add legacy API aliases to store (`goBack: s.navigateBack`, `navigateWithHistory: s.navigateToView`), (2) Remove Provider from AppProviders, (3) Add migration comment block in contexts/index.ts explaining new import paths, (4) Update JSDoc in store with Story reference. Architecture validated: Single Source of Truth, Zustand-first pattern) |
| 2026-01-30 | Nested directory staging verification pattern (14e-42 atlas-code-review: Story created `src/features/categories/utils/` AND `tests/unit/features/categories/utils/` - BOTH showed `??` UNTRACKED. Pattern: When story creates NEW subdirectories in existing features, check for MULTIPLE `??` entries - each new directory tree needs separate `git add`), Pure utility extraction with DI (14e-42 APPROVED: `applyItemNameMappings` follows 14e-8a pattern - pure function with `FindItemNameMatchFn` parameter, 16 boundary tests, duplicate removed from subhandlers.ts. Architecture validated: Feature Utils Pattern, Dependency Injection, Single Source of Truth) |
| 2026-01-29 | Settings Store Extension Pattern (14e-35 atlas-code-review APPROVED: Extend existing Zustand stores by adding state/actions/selectors - individual selectors for render isolation, combined useShallow selector for related fields, dual writes for backward compatibility with plain localStorage keys. Pattern enables incremental store consolidation without breaking consumers), Placeholder handler documentation pattern (14e-29d atlas-code-review: handleReduceBatch stub evolved from "stub - 14e-29c" comment to explicit JSDoc explaining WHY it's a no-op - deprioritized UX flow, user workarounds documented. Pattern: Placeholder handlers need explicit JSDoc explaining (1) why it exists, (2) why it's not implemented, (3) what user should do instead, (4) future implementation notes. Prevents "is this a bug?" confusion), Test count accuracy in story docs (14e-29d atlas-code-review: Story claimed 6807 tests but actual run showed 5962 - test counts can drift across multi-day development. Pattern: Re-run tests and update count in story BEFORE marking for review, not during initial implementation), Comprehensive staging audit before review (14e-33 atlas-code-review: Found 2 HIGH severity issues - AC1 implementation (`src/components/scan/BatchUploadPreview.tsx`) and tests (`tests/unit/components/scan/BatchUploadPreview.test.tsx`) showed ` M` (unstaged). Story marked "done" but implementation would be LOST on commit. Pattern: Run `git status --porcelain` before marking story done - ALL claimed files must have `A ` `M ` or `D ` prefix (staged). Files with ` M` `??` `AM` `MM` need `git add` before commit. Verify: no space before status letter = staged correctly), Mobile-first touch target pattern (14e-33: Replace hover-only opacity `opacity-0 group-hover:opacity-100` with always-visible buttons + `min-w-[44px] min-h-[44px]` for 44px touch targets + semi-transparent background for visibility. Desktop hover patterns don't work on touch devices), Zustand store synchronous access in callbacks (14e-33: Use `useBatchReviewStore.getState().items` for synchronous state checks in useCallback handlers - React context state (`scanState.batchReceipts`) may be stale when callback executes. Pattern: When state exists in both Zustand and Context, check Zustand via `getState()` as source of truth), Stale state recovery pattern (14e-33: Auto-detect corrupted/cleared localStorage by checking `!hadItems && isEmpty` - user lands on view but never had data. Pattern: Add recovery case in auto-complete useEffect to reset and navigate to safe state), Dual-store sync pattern (14e-33: When operating on items that exist in TWO stores (batch-review store + scan store), call BOTH store actions: `batchReviewActions.discardItem(id)` AND `scanActions.discardBatchReceipt(id)`. Prevents index misalignment and duplicate saves), Single source of truth migration pattern (14e-34a APPROVED: Eliminate duplicate useState by consolidating to Zustand store. Pattern: (1) Add selector to store (useScanImages), (2) Update component to use selector instead of props, (3) Map store action to familiar local name in hooks (setImages: setBatchImages), (4) Add comment breadcrumbs "Story 14e-34a:" explaining migration rationale, (5) Update tests with store mock pattern: `let mockImages: string[]` + `vi.mock('@/features/scan/store')`), Cross-feature import documentation pattern (14e-34a APPROVED: When one feature imports from another (batch-review imports useScanStore), add explicit JSDoc comment explaining WHY the cross-feature import is intentional and the alternative (state duplication) that was rejected. Location: useBatchReviewHandlers.ts:32-35), New feature directory staging pattern (14e-36a atlas-code-review APPROVED: When creating new feature directories like `src/features/{name}/`, the ENTIRE directory shows as single `??` in git status. Must run `git add src/features/{name}/` to stage all files - individual files won't show until directory is tracked. Simple setter stores (no phase guards) follow useBatchReviewStore pattern but without lifecycle complexity) |
