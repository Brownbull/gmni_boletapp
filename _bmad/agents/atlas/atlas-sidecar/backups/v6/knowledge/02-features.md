...
### Story Created - 14e-39-trust-prompt-credit-feature (2026-01-29)

**Summary:** Move trust prompt state (showTrustPrompt, trustPromptData, shouldTriggerCreditCheck) from App.tsx to CreditFeature

**User Value:** Centralized credit/trust state management, ~30 lines removed from App.tsx

**Workflow Touchpoints:**
- Workflow #8 (Trust Merchant Flow): DIRECT - Trust prompt state controls flow entry
- Workflow #1 (Scan Receipt Flow): INDIRECT - Trust prompt shown after save
- Workflow #2 (Quick Save Flow): INDIRECT - Credit check trigger affects path
- Workflow #9 (Scan Lifecycle): INDIRECT - Part of scan completion

**Atlas Enhancements:**
- AC6: AppOverlays integration (TrustMerchantPrompt location decision)
- AC7: Handler callback updates (processScan context changes)

**Files Modified:**
- `src/features/credit/state/useCreditState.ts` - Add trust state
- `src/features/credit/CreditFeature.tsx` - Manage TrustMerchantPrompt
- `src/components/App/AppOverlays.tsx` - Remove trust props
- `src/App.tsx` - Remove trust useState

**Source:** `docs/sprint-artifacts/epic14e-feature-architecture/stories/14e-39-trust-prompt-credit-feature.md`

### Story Ready - 14e-42-extract-item-name-mappings (2026-01-29)

**Summary:** Extract applyItemNameMappings to categories feature + remove duplicate from subhandlers.ts

**User Value:** Single source of truth for item name mapping logic, App.tsx reduced by ~40 lines

**Workflow Touchpoints:**
- Workflow #1 (Scan Receipt): DIRECT - Applied in processScan after OCR
- Workflow #2 (Quick Save): INDIRECT - Auto-save uses learned mappings
- Workflow #3 (Batch Processing): DIRECT - Each batch receipt uses mappings
- Workflow #5 (Learning Flow): DIRECT - This IS the application of learned mappings

**Key Insight:** Duplicate code identified in subhandlers.ts from Story 14e-8b - removed as part of this story (AC5)

**Source:** `docs/sprint-artifacts/epic14e-feature-architecture/stories/14e-42-extract-item-name-mappings.md`

### Story Created - 14e-43-processscan-simplification (2026-01-29)

**Summary:** Reduce processScan Dependency Injection - simplify handler API from 50+ to ~20 params

**User Value:** Cleaner handler API, ~100 lines removed from App.tsx wrapper, better testability

**Workflow Touchpoints:**
- Workflow #1 (Scan Receipt Flow): DIRECT - processScan is core handler
- Workflow #2 (Quick Save Flow): DIRECT - routes through processScan
- Workflow #3 (Batch Processing Flow): DIRECT - each batch receipt uses processScan
- Workflow #9 (Scan Request Lifecycle): DIRECT - orchestrates SCANNING→REVIEWING phase

**Atlas Enhancements Applied:**
- AC5: Test migration strategy (callback mocks → store mocks)
- AC6: Store import pattern (non-React direct access)
- AC7: Callback deprecation audit
- AC8: Workflow integrity tests

**Dependencies:** 14e-34a, 14e-36, 14e-40, 14e-41, 14e-42

**Source:** `docs/sprint-artifacts/epic14e-feature-architecture/stories/14e-43-processscan-simplification.md`
