...
- [x] **AC1:** `useScanHandlers.ts` reduced to <800 lines (from 957) — 552 lines
- [x] **AC2:** Each extracted file is <400 lines — scanHandlerTypes.ts=258, useScanFlowRouter.ts=291
- [x] **AC3:** Behavior snapshot: all existing tests pass before AND after extraction (including `tests/unit/features/scan/hooks/useScanHandlers.test.ts`)
- [x] **AC4:** No new functionality added -- pure decomposition
- [x] **AC5:** The `UseScanHandlersResult` return type remains unchanged -- consumers see the same API
- [x] **AC6:** `npm run test:quick` passes with 0 failures — 6962 tests, 0 failures
...
- [x] **AC-ARCH-LOC-1:** Type definitions at `src/features/scan/hooks/scanHandlerTypes.ts`
- [x] **AC-ARCH-LOC-2:** Flow routing sub-hook at `src/features/scan/hooks/useScanFlowRouter.ts`
- [x] **AC-ARCH-LOC-3:** Flow router tests at `tests/unit/features/scan/hooks/useScanFlowRouter.test.ts`

### Pattern Requirements

- [x] **AC-ARCH-PATTERN-1:** All extracted files use `@/` or `@features/` path aliases for external imports -- zero `../../` relative imports
- [x] **AC-ARCH-PATTERN-2:** `useScanHandlers.ts` imports extracted modules via relative `./` paths (same directory)
- [x] **AC-ARCH-PATTERN-3:** `scanHandlerTypes.ts` contains ONLY type definitions, interfaces, and type re-exports -- no runtime code, no React imports
- [x] **AC-ARCH-PATTERN-4:** `useScanFlowRouter.ts` follows React custom hook pattern (function name starts with `use`, returns object with handlers)
- [x] **AC-ARCH-PATTERN-5:** `useScanFlowRouter` receives its dependencies via a props object -- NOT via context or store access
- [x] **AC-ARCH-PATTERN-6:** The barrel export at `src/features/scan/hooks/index.ts` continues to export `useScanHandlers`, `UseScanHandlersProps`, and `UseScanHandlersResult` -- sub-files are internal implementation details

### Anti-Pattern Requirements (Must NOT Happen)

- [x] **AC-ARCH-NO-1:** No circular dependency -- `useScanFlowRouter.ts` must NOT import from `./useScanHandlers` or from `@features/scan/hooks` barrel
- [x] **AC-ARCH-NO-2:** No new `console.log` statements in extracted files (existing `console.error` and `console.warn` are preserved as-is)
- [x] **AC-ARCH-NO-3:** No `: any` types in new files -- the 2 existing `eslint-disable` for `: any` in `UseScanHandlersProps` stay in `scanHandlerTypes.ts`
- [x] **AC-ARCH-NO-4:** No hook call order changes -- `useCallback` calls in `useScanHandlers` that depend on `useScanFlowRouter` outputs must appear AFTER the `useScanFlowRouter()` call
- [x] **AC-ARCH-NO-5:** No barrel modification -- `src/features/scan/hooks/index.ts` does NOT export `useScanFlowRouter` or `scanHandlerTypes` (they are internal)
...
- [x] 1.2 Run `npx vitest run tests/unit/features/scan/hooks/useScanHandlers.test.ts` and confirm all pass
- [x] 1.3 Count current lines: `wc -l src/features/scan/hooks/useScanHandlers.ts` (expect 957)
- [x] 1.4 Verify no circular deps in scan hooks: `npx madge --circular src/features/scan/hooks/`

### Task 2: Extract type definitions into scanHandlerTypes.ts

- [x] 2.1 Create `src/features/scan/hooks/scanHandlerTypes.ts`
- [x] 2.2 Move `BatchSession` interface
- [x] 2.3 Move `ItemNameMapping` interface
- [x] 2.4 Move `ItemNameMatchResult` interface
- [x] 2.5 Move `SessionContextData` interface
- [x] 2.6 Move `ScanOverlayState` interface
- [x] 2.7 Move `UseScanHandlersProps` interface with its doc comment (including the 2 existing `eslint-disable` comments)
- [x] 2.8 Move `UseScanHandlersResult` interface with its doc comment
- [x] 2.9 Add necessary type imports to `scanHandlerTypes.ts`: `User` from `firebase/auth`, `Firestore` from `firebase/firestore`, `Transaction`/`TransactionItem` from `@/types/transaction`, `UserPreferences` from `@/services/userPreferencesService`, `Insight`/`UserInsightProfile`/`LocalInsightCache` from `@/types/insight`, `View` from `@app/types`, `TrustPromptEligibility` from `@/types/trust`, dialog types from `@/types/scanStateMachine`, `ToastMessage` from `@/shared/hooks`
- [x] 2.10 Update `useScanHandlers.ts`: replace inline types with `import type { ... } from './scanHandlerTypes'`
- [x] 2.11 Run `npx tsc --noEmit` -- fix any type errors

### Task 3: Extract flow routing into useScanFlowRouter.ts

- [x] 3.1 Create `src/features/scan/hooks/useScanFlowRouter.ts`
- [x] 3.2 Define `UseScanFlowRouterProps` interface with the subset of props needed: `user`, services, `userPreferences`, `categoryMappings`, `findMerchantMatch`, `applyCategoryMappings`, `applyItemNameMappings` (passed as callback), `incrementMappingUsage`, `incrementMerchantMappingUsage`, `incrementItemNameMappingUsage`, `checkTrusted`, `dispatchProcessSuccess`, `showScanDialog`, `setCurrentTransaction`, `setToastMessage`, `setView`, `setSkipScanCompleteModal`, `setAnimateEditViewItems`, `t`
- [x] 3.3 Move `continueScanWithTransaction` handler into the new hook
- [x] 3.4 Move `proceedAfterCurrencyResolved` handler into the new hook
- [x] 3.5 Add imports for scan state machine types, confidence check utils, and firestore services as needed
- [x] 3.6 Return `{ continueScanWithTransaction, proceedAfterCurrencyResolved }` from the hook
- [x] 3.7 Update `useScanHandlers.ts`: call `useScanFlowRouter()` at the correct position (after `applyItemNameMappings` useCallback, before currency/total mismatch handlers) and destructure results
- [x] 3.8 Run `npx tsc --noEmit` -- fix any type errors

### Task 4: Add tests for useScanFlowRouter

- [x] 4.1 Create `tests/unit/features/scan/hooks/useScanFlowRouter.test.ts`
- [x] 4.2 Create `createDefaultFlowRouterProps()` factory matching `UseScanFlowRouterProps`
- [x] 4.3 Test `continueScanWithTransaction`: applies category mappings, applies merchant mappings, applies item name mappings, checks currency mismatch, shows quick save dialog, auto-saves for trusted merchants, falls through to edit view for low confidence
- [x] 4.4 Test `proceedAfterCurrencyResolved`: trusted merchant auto-save, quick save dialog, edit view fallback
- [x] 4.5 Run `npx vitest run tests/unit/features/scan/hooks/useScanFlowRouter.test.ts` -- all pass

### Task 5: Verify extraction and run full test suite

- [x] 5.1 Count final `useScanHandlers.ts` lines: `wc -l src/features/scan/hooks/useScanHandlers.ts` (target: <800)
- [x] 5.2 Verify all new files are <400 lines each
- [x] 5.3 Verify no `../../` imports: `grep -rE "from '\.\./\.\." src/features/scan/hooks/scanHandlerTypes.ts src/features/scan/hooks/useScanFlowRouter.ts` returns 0
- [x] 5.4 Verify no circular deps: `npx madge --circular src/features/scan/hooks/`
- [x] 5.5 Run `npx vitest run tests/unit/features/scan/hooks/useScanHandlers.test.ts` -- all pass unchanged
...
|----------|-------------|----------|--------|
| TD-15b-19 | Split useScanFlowRouter.test.ts (391L) into 2 files under 300L guideline | LOW | CREATED |
| -- | console.error may leak Firestore paths in catch blocks (pre-existing) | LOW | NOT TRACKED -- cross-cutting concern |
| -- | Advisory TOCTOU gap checkTrusted to addTransaction (pre-existing) | LOW | NOT TRACKED -- low risk, trust is advisory |
...
| 2026-02-25 | Code review: APPROVE 9/10. Created TD-15b-19 (test file split). 2 pre-existing LOW items noted but not tracked (cross-cutting). |
...
- **Architectural ACs:** 11/11 PASS
- **Functional ACs:** 6/6 PASS
- **Test Health:** 6962 tests, 0 failures
- **Action Items:** 0 quick fixes, 1 TD story created (TD-15b-19)
