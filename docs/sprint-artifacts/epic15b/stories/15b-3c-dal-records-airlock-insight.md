...
- [x] **AC1:** `usePersonalRecords.ts` uses `useRecordsRepository` for the `storePersonalRecord` call; pure functions `detectAndFilterRecords` and `getCurrentWeekId` remain as direct service imports (they are not Firestore operations)
- [x] **AC2:** Airlock service has zero direct consumers outside the repository -- VERIFY only, no migration needed
- [x] **AC3:** `useInsightProfile.ts` uses `useInsightProfileRepository` for all 5 Firestore calls
- [x] **AC4:** `InsightsView.tsx` uses `useInsightProfileRepository` instead of `getUserInsightProfile` from `insightEngineService`
- [x] **AC5:** The `getUserInsightProfile` re-export in `insightEngineService.ts` is removed after consumers are migrated
- [x] **AC6:** No direct `insightProfileService` imports remain in non-repository files after migration
- [x] **AC7:** Each consumer migrated individually with tests passing between migrations
...
- [x] **AC-ARCH-LOC-1:** `src/repositories/recordsRepository.ts` -- verified complete, no changes needed
- [x] **AC-ARCH-LOC-2:** `src/repositories/airlockRepository.ts` -- verified complete; zero external consumers
- [x] **AC-ARCH-LOC-3:** `src/repositories/insightProfileRepository.ts` -- verified complete, no changes needed
- [x] **AC-ARCH-LOC-4:** `src/hooks/usePersonalRecords.ts` imports repository hook from `@/repositories`
- [x] **AC-ARCH-LOC-5:** `src/features/insights/hooks/useInsightProfile.ts` imports repository hook from `@/repositories`
- [x] **AC-ARCH-LOC-6:** `src/features/insights/views/InsightsView.tsx` uses repository hook, not `getUserInsightProfile` from engine service

### Pattern Requirements

- [x] **AC-ARCH-PATTERN-1:** Consumers obtain repository instance via `useXxxRepository()` hook, not by constructing directly
- [x] **AC-ARCH-PATTERN-2:** Repository hook returns `null` when not authenticated; consumers guard on null before calling methods
- [x] **AC-ARCH-PATTERN-3:** Pure computation functions (`detectAndFilterRecords`, `getCurrentWeekId`) remain as direct service imports -- repositories only wrap data access
- [x] **AC-ARCH-PATTERN-4:** Insight profile repository delegates to TOCTOU-safe `runTransaction()` service functions (verified, not re-implemented)

### Anti-Pattern Requirements (Must NOT Happen)

- [x] **AC-ARCH-NO-1:** Do NOT wrap pure computation functions in repositories (no `detectAndFilterRecords` in `IRecordsRepository`)
- [x] **AC-ARCH-NO-2:** Do NOT introduce new `useAuth()` calls in files that already receive auth context via props -- check `usePersonalRecords` which receives `db, userId, appId` as props
- [x] **AC-ARCH-NO-3:** Must NOT bypass TOCTOU-safe `runTransaction` wrappers in the insight profile repository
- [x] **AC-ARCH-NO-4:** Must NOT remove the `insightEngineService.ts` re-export before `InsightsView.tsx` is migrated
...
- [x] 1.2 `grep -rn "from.*recordsService" src/ --include="*.ts" --include="*.tsx" | grep -v repositories/ | grep -v "services/index.ts"` -- confirm consumer list
- [x] 1.3 `grep -rn "from.*airlockService" src/ --include="*.ts" --include="*.tsx" | grep -v repositories/` -- confirm 0 external consumers
- [x] 1.4 `grep -rn "from.*insightProfileService" src/ --include="*.ts" --include="*.tsx" | grep -v repositories/` -- confirm consumer list
- [x] 1.5 `grep -rn "getUserInsightProfile" src/ --include="*.ts" --include="*.tsx"` -- confirm InsightsView and insightEngineService

### Task 2: Migrate usePersonalRecords.ts

- [x] 2.1 In `src/hooks/usePersonalRecords.ts`: add `import { useRecordsRepository } from '@/repositories'`; remove `storePersonalRecord` from recordsService import (keep pure function imports)
- [x] 2.2 Replace `storePersonalRecord(db, userId, appId, record)` with `recordsRepo?.store(record)` where `recordsRepo = useRecordsRepository()`; preserve null guard (`if (!recordsRepo) return`)
- [x] 2.3 Note: Keep `{db, userId, appId}` interface for minimal ripple to `App.tsx` (Option A -- see Dev Notes)
- [x] 2.4 Update `tests/unit/hooks/usePersonalRecords.test.ts`: mock `useRecordsRepository` from `@/repositories` instead of `storePersonalRecord` from service
- [x] 2.5 Run `npx vitest run tests/unit/hooks/usePersonalRecords.test.ts` -- all pass

### Task 3: Migrate useInsightProfile.ts

- [x] 3.1 In `src/features/insights/hooks/useInsightProfile.ts`: add `import { useInsightProfileRepository } from '@/repositories'`; remove 5 insightProfileService imports
- [x] 3.2 Replace service calls with repository equivalents:
...
- [x] 3.3 Update `tests/unit/features/insights/hooks/useInsightProfile.test.ts`: mock `useInsightProfileRepository` from `@/repositories`
- [x] 3.4 Run `npx vitest run tests/unit/features/insights/hooks/useInsightProfile.test.ts` -- all pass

### Task 4: Migrate InsightsView.tsx and clean up re-export

- [x] 4.1 In `src/features/insights/views/InsightsView.tsx`: add `import { useInsightProfileRepository } from '@/repositories'`; remove `getUserInsightProfile` import from `insightEngineService`
- [x] 4.2 Replace the standalone `getUserInsightProfile(services.db, user.uid, services.appId)` call (in useEffect) with `insightProfileRepo?.get()` or `insightProfileRepo?.getOrCreate()`; call `useInsightProfileRepository()` at component level
- [x] 4.3 In `src/features/insights/services/insightEngineService.ts`: remove the `getUserInsightProfile` re-export line; verify no other consumers
- [x] 4.4 Update `tests/unit/features/insights/views/InsightsView.test.tsx`: update mocks
- [x] 4.5 Run `npx vitest run tests/unit/features/insights/views/InsightsView.test.tsx` -- all pass

### Task 5: Verify and run full test suite

- [x] 5.1 `grep -rn "from.*recordsService\|from.*airlockService\|from.*insightProfileService" src/ --include="*.ts" --include="*.tsx" | grep -v repositories/ | grep -v "services/index.ts"` -- must return 0 lines
- [x] 5.2 `grep -rn "getUserInsightProfile" src/ --include="*.ts" --include="*.tsx"` -- must return 0 lines
- [x] 5.3 Run `npm run test:quick` -- must pass with 0 failures
...
| Overall | 9/10 APPROVE |
| Findings | 3 (0 CRITICAL, 0 HIGH, 2 LOW, 1 INFO) |
| Quick Fixes | 1 (airlock zero-consumer verification) |
| Deferred | 2 LOW (logging consistency, test assertion gap — too small for standalone TD) |
| Tracked Elsewhere | usePersonalRecords interface cleanup → 15b-4f-app-tsx-fanout-reduction |
