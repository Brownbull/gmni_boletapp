...
- [ ] **AC1:** All source files under `src/features/batch-review/` use direct imports (`../hooks/useBatchCapture`) or canonical feature paths (`@features/batch-review/hooks/useBatchCapture`) — zero references to `@/hooks/useBatch*` shim paths
- [ ] **AC2:** All test files under `tests/unit/features/batch-review/` mock and import from canonical paths matching updated source imports
- [ ] **AC3:** `npm run test:quick` passes with 0 failures
- [ ] **AC4:** Grep verification: `grep -r '@/hooks/useBatch' src/features/batch-review/` returns 0
- [ ] **AC5:** Grep verification: `grep -r '@/hooks/useBatch' tests/unit/features/batch-review/` returns 0

## Tasks / Subtasks

### Task 1: Update source file imports (5 files)

- [ ] 1.1 `src/features/batch-review/components/BatchCaptureUI.tsx` — `@/hooks/useBatchCapture` → `../hooks/useBatchCapture`
- [ ] 1.2 `src/features/batch-review/components/BatchProcessingView.tsx` — `@/hooks/useBatchProcessing` → `../hooks/useBatchProcessing`
- [ ] 1.3 `src/features/batch-review/components/BatchThumbnailStrip.tsx` — `@/hooks/useBatchCapture` (type import) → `../hooks/useBatchCapture`
- [ ] 1.4 `src/features/batch-review/views/BatchCaptureView.tsx` — `@/hooks/useBatchCapture` → `../hooks/useBatchCapture`
- [ ] 1.5 `src/features/batch-review/views/BatchReviewView.tsx` — `@/hooks/useBatchReview` → `../hooks/useBatchReview`

### Task 2: Update test mock/import paths (4 files, 8 references)

- [ ] 2.1 `tests/unit/features/batch-review/components/BatchCaptureUI.test.tsx` — vi.mock + 2 imports
- [ ] 2.2 `tests/unit/features/batch-review/components/BatchProcessingView.test.tsx` — vi.mock + import
- [ ] 2.3 `tests/unit/features/batch-review/components/BatchThumbnailStrip.test.tsx` — type import
- [ ] 2.4 `tests/unit/features/batch-review/views/BatchReviewView.test.tsx` — vi.mock + import

### Task 3: Verification

- [ ] 3.1 Run `npx vitest run tests/unit/features/batch-review/` — all 538 tests pass
- [ ] 3.2 Run grep verifications (AC4, AC5)
...
- Files affected: 5 source + 4 test = 9 files
