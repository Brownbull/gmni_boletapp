...
1. **Firebase Hosting** is correctly configured with SPA rewrites (`**` → `/index.html`)
...
```
Share Code Generated → Link Shared → [THIS STORY: URL Detection] → Join Dialog → Group Mode Active
                                   ↳ (if unauth) → Login → Resume Join
```
...
4. If authenticated → show dialog immediately
5. If not authenticated → store in sessionStorage, wait for auth
...
- **All acceptance criteria satisfied**:
  ...
  - AC2: Authenticated User Flow ✅
  - AC3: Unauthenticated User Flow ✅
  - AC4: Join Confirmation Dialog ✅
  - AC5: Error Handling ✅
  - AC6: URL Cleanup ✅
