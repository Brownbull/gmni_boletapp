...
describe('reconcileItemsTotal', () => {
  ...
  it('should allow small discrepancies (< $1)', () => {
    const items: TransactionItem[] = [{ name: 'Item 1', totalPrice: 9999.5, qty: 1 }];
  ...
  it('should handle floating point rounding correctly', () => {
    const items: TransactionItem[] = [
      { name: 'Item 1', totalPrice: 33.33, qty: 1 },
      { name: 'Item 2', totalPrice: 33.33, qty: 1 },
      { name: 'Item 3', totalPrice: 33.33, qty: 1 },
    ];
  ...
  it('should add surplus item when receipt > items', () => {
    const items: TransactionItem[] = [{ name: 'Item 1', totalPrice: 8000, qty: 1 }];
    ...
    expect(adjustmentItem.totalPrice).toBe(2000);
  });
  ...
  it('should add discount item when items > receipt', () => {
    const items: TransactionItem[] = [{ name: 'Item 1', totalPrice: 12000, qty: 1 }];
  ...
  it('should handle zero receipt total', () => {
    const items: TransactionItem[] = [{ name: 'Item 1', totalPrice: 5000, qty: 1 }];
  ...
  it('should treat non-number prices as zero', () => {
    ...
    const items = [
      { name: 'Valid Item', totalPrice: 5000, qty: 1 },
      { name: 'Invalid Item', totalPrice: undefined as unknown as number, qty: 1 },
      { name: 'Null Item', totalPrice: null as unknown as number, qty: 1 },
    ];
