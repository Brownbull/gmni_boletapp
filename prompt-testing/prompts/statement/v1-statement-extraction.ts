/**
 * V1 Statement Extraction Prompt
 *
 ...
 *
 * Validated against 12 CMR Falabella PDFs (100% success, 1,100 transactions).
 */
...
function buildV1StatementPrompt(): string {
...
9. RETURN the English KEY (e.g., "Supermarket", "GasStation"), NOT the Spanish label`;
}
...
export const STATEMENT_PROMPT_V1: StatementPromptConfig = {
  id: 's1-statement-extraction',
  name: 'Statement Extraction V1',
  description:
    'Chilean credit card statement extraction. Multi-transaction output with statement metadata. ' +
    'Validated on CMR Falabella (12 PDFs, 1,100 txns, 100% success).',
  version: '1.0.0',
  createdAt: '2026-03-11',
  prompt: buildV1StatementPrompt(),
};

/**
 * Build a complete V1 statement prompt with all variables replaced.
 */
export function buildCompleteV1StatementPrompt(options: {
  date: string;
}): string {
  return STATEMENT_PROMPT_V1.prompt.replaceAll('{{date}}', options.date);
}
